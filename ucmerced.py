...
print("🚀 AURA-RS Production Version - UC Merced Land Use Dataset")
...
def safe_load_image_with_resize(image_path: str, patch_size: int = 224) -> Optional[Image.Image]:
...
class AuraConfig:
    """
    AURA-RS Configuration for UC Merced Land Use Dataset
    ...
    # Improved LoRA hyperparameters for better domain adaptation
    ...
    LORA_SAMPLES_PER_CLASS: int = 120  # Increased for better adaptation
    ...
    def switch_to_production_mode(self):
        """Quick switch to production mode for robust results"""
        print("🔄 Switching to production mode...")
        self.DEVELOPMENT_MODE = False
        self.RANDOM_SEEDS = [42, 123, 456]
        self.ACADEMIC_ANALYSIS['statistical_tests'] = True
        self.ACADEMIC_ANALYSIS['confidence_intervals'] = True
        self.ABLATION_STUDIES['run_ablation'] = True
        self.VISUALIZATION['examples_per_category'] = 5
        print("✅ Production mode activated - Ready for robust experiments!")
        
    def enable_ablation_studies(self):
        """Enable comprehensive ablation studies"""
        print("🧪 Enabling ablation studies...")
        self.ABLATION_STUDIES['run_ablation'] = True
        self.ABLATION_STUDIES['temperature_scaling'] = [0.03, 0.05, 0.07, 0.1]
        self.ABLATION_STUDIES['normalization_methods'] = ['l2', 'none']
        print("✅ Ablation studies enabled!")
...
class RemoteSensingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = safe_load_image_with_resize(item['image_path'], self.config.PATCH_SIZE)
        ...
        return {
            'image': image,
            'text': f"A satellite image of {item['class_name']}",
        ...
        try:
            ...
            for epoch in range(self.config.LORA_EPOCHS):
                ...
                for i in range(0, len(adaptation_data), self.config.BATCH_SIZE):
                    ...
                    for item in batch:
                        try:
                            img = safe_load_image_with_resize(item['image_path'], self.config.PATCH_SIZE)
                            if img is None:
                                continue
...
class DataManager:
    ...
    def load_dataset(self) -> bool:
        try:
            if self.config.DATASET.lower() == "ucmerced":
                self.dataset_path = self._download_ucmerced_dataset()
                return self._load_from_directory()
            elif self.config.DATASET.lower() == "dior":
                self.dataset_path = self._download_dior_dataset()
                return self._load_dior_format()
            else:
    ...
    def _download_ucmerced_dataset(self) -> str:
        print("Downloading UC Merced dataset from Kaggle...")
        try:
    ...
    def _load_from_directory(self) -> bool:
        possible_paths = [
            ...
            os.path.join(self.dataset_path, "UCMerced_LandUse", "Images"),
            os.path.join(self.dataset_path, "uc-merced-land-use-dataset", "Images")
        ]
        ...
        if not images_path:
            print(f"Images folder not found. Tried: {possible_paths}")
            return False
            
        print(f"Loading UC Merced dataset: {images_path}")
        total_images = 0
        class_counts = defaultdict(int)
        
        for class_name in self.config.ALL_CLASSES:
            class_path = os.path.join(images_path, class_name)
            if not os.path.exists(class_path):
                print(f"Class folder not found: {class_name}")
                continue
                
            image_files = [f for f in os.listdir(class_path) if f.lower().endswith(('.jpg', '.tif'))]
            for image_file in image_files:
                if class_counts[class_name] >= self.config.MAX_SAMPLES_PER_CLASS:
                    break
        ...
        print(f"Loaded {total_images} samples across {len(class_counts)} classes")
        return True
    
    def _download_dior_dataset(self) -> str:
        """Download DIOR dataset - placeholder for future implementation"""
        print("📊 DIOR dataset support - using local DIOR_RSVG path")
        # Check if DIOR_RSVG exists in data folder
        dior_path = os.path.join("data", "RSVG-pytorch", "DIOR_RSVG")
        if os.path.exists(dior_path):
            print(f"✅ Found DIOR dataset: {dior_path}")
            return dior_path
        else:
            print(f"❌ DIOR dataset not found at: {dior_path}")
            print("💡 Please place DIOR_RSVG dataset in data/RSVG-pytorch/ folder")
            raise FileNotFoundError("DIOR dataset not found")
    
    def _load_dior_format(self) -> bool:
        """Load DIOR dataset in RSVG format"""
        print(f"Loading DIOR dataset: {self.dataset_path}")
        
        images_path = os.path.join(self.dataset_path, "JPEGImages")
        ...
        if not os.path.exists(images_path) or not os.path.exists(annotations_path):
            print(f"Missing DIOR folders: {images_path} or {annotations_path}")
            return False
        
        # Load from train.txt, val.txt, test.txt if available
        splits_files = ['train.txt', 'val.txt', 'test.txt']
        available_images = set()
        
        for split_file in splits_files:
            split_path = os.path.join(self.dataset_path, split_file)
        ...
        # DIOR classes mapped to our novel classes
        dior_class_mapping = {
            'airplane': 'airplane',
        ...
        for img_id in list(available_images)[:1000]:  # Limit for testing
            img_path = os.path.join(images_path, f"{img_id}.jpg")
            if os.path.exists(img_path):
                # For now, assign random classes for testing
                # In real implementation, parse XML annotations
                import random
                random.seed(42)
                class_name = random.choice(list(dior_class_mapping.values()))
        ...
        print(f"Loaded {total_images} DIOR samples across {len(class_counts)} classes")
        return True
...
def evaluate_model(model: AuraModel, test_data: List[Dict], query_embedding: torch.Tensor, 
    ...
    for item in all_test_samples:
        try:
            img = safe_load_image_with_resize(item['image_path'], config.PATCH_SIZE)
            if img is not None:
...
def compute_validation_threshold_simple(model: AuraModel, validation_data: List[Dict], 
    ...
    with torch.no_grad():
        for item in val_samples:
            try:
                img = safe_load_image_with_resize(item['image_path'], model.config.PATCH_SIZE)
                if img is None:
                    continue
...
def run_experiments(config: AuraConfig) -> pd.DataFrame:
    print("Starting AURA-RS experiments on UC Merced dataset")
...
def generate_academic_report(results_df: pd.DataFrame, stats_results: Dict, config: AuraConfig, bootstrap_results: Dict = None) -> str:
    ...
    if len(scenario_results) > 0:
        ...
        report.append("\\caption{Performance comparison on UC Merced dataset}")
...
def save_results_with_fallback(results_df: pd.DataFrame, config: AuraConfig, timestamp: int) -> Optional[str]:
    ...
    try:
        results_file = os.path.join(config.RESULTS_PATH, f"ucmerced_results_{timestamp}.csv")
        ...
        try:
            fallback_file = f"ucmerced_results_{timestamp}.csv"
...
def perform_statistical_analysis(results_df: pd.DataFrame, config: AuraConfig, timestamp: int) -> Dict:
    ...
    try:
        ...
        if stats_results:
            stats_file = os.path.join(config.RESULTS_PATH, f"ucmerced_stats_{timestamp}.json")
    ...
    try:
        ...
        if bootstrap_results:
            bootstrap_file = os.path.join(config.RESULTS_PATH, f"ucmerced_bootstrap_{timestamp}.json")
...
def quick_lora_comparison():
    """Quick function to compare old vs new LoRA hyperparameters"""
    print("🔄 Quick LoRA Hyperparameter Comparison")
    print("=" * 50)
    
    # Test with improved hyperparameters
    config = AuraConfig()
    print("🆕 New LoRA Config:")
    print(f"   Rank: {config.LORA_RANK}, Alpha: {config.LORA_ALPHA}")
    print(f"   LR: {config.LORA_LR}, Epochs: {config.LORA_EPOCHS}")
    print(f"   Samples: {config.LORA_SAMPLES_PER_CLASS}")
    
    results_df = run_experiments(config)
    
    # Save comparison results
    timestamp = int(time.time())
    comparison_file = os.path.join(config.RESULTS_PATH, f"lora_comparison_{timestamp}.csv")
    results_df.to_csv(comparison_file, index=False)
    print(f"💾 LoRA comparison saved: {comparison_file}")
    
    return results_df

def quick_production_run():
    """Quick function to run full production experiments"""
    print("🔬 Quick Production Run")
    print("=" * 50)
    
    config = AuraConfig()
    config.switch_to_production_mode()
    
    results_df = run_experiments(config)
    
    # Enhanced analysis for production
    timestamp = int(time.time())
    production_file = os.path.join(config.RESULTS_PATH, f"production_results_{timestamp}.csv")
    results_df.to_csv(production_file, index=False)
    print(f"💾 Production results saved: {production_file}")
    
    return results_df

def quick_ablation_study():
    """Quick function to run ablation studies"""
    print("🧪 Quick Ablation Study")
    print("=" * 50)
    
    config = AuraConfig()
    config.enable_ablation_studies()
    
    results_df = run_experiments(config)
    
    # Focus on ablation results
    ablation_results = results_df[results_df['result_type'] == 'ablation'] if 'result_type' in results_df.columns else pd.DataFrame()
    if len(ablation_results) > 0:
        print("\n📊 Ablation Study Results:")
        for temp in ablation_results['temperature'].unique():
            temp_data = ablation_results[ablation_results['temperature'] == temp]
            mean_f1 = temp_data['f1'].mean()
            print(f"   Temperature {temp}: F1 = {mean_f1:.3f}")
    
    return results_df

def quick_dior_test():
    """Quick function to test DIOR dataset"""
    print("📊 Quick DIOR Dataset Test")
    print("=" * 50)
    
    config = AuraConfig()
    config.DATASET = "dior"
    
    # Check if DIOR data exists
    dior_path = os.path.join("data", "RSVG-pytorch", "DIOR_RSVG")
    if not os.path.exists(dior_path):
        print(f"❌ DIOR dataset not found at: {dior_path}")
        print("💡 Using UC Merced instead")
        config.DATASET = "ucmerced"
    
    try:
        results_df = run_experiments(config)
        print("✅ DIOR test completed!")
        return results_df
    except Exception as e:
        print(f"❌ DIOR test failed: {e}")
        print("💡 Falling back to UC Merced")
        config.DATASET = "ucmerced"
